...
class Store:
    ...
    def audit(self, company_id: str | None, event: str, entity_type: str, entity_id: str, payload: dict[str, Any]) -> None:
        self.conn.execute("INSERT INTO audit_log(company_id,event,entity_type,entity_id,payload_json,created_at) VALUES(?,?,?,?,?,?)",
                          (company_id, event, entity_type, entity_id, json.dumps(payload), now()))
        self.conn.commit()
    ...
    def claim_task(self) -> dict[str, Any] | None:
        ...
        with self.lock, self.conn:
            row = self.conn.execute("SELECT * FROM tasks WHERE status='queued' AND (available_at IS NULL OR available_at <= ?) ORDER BY priority ASC, created_at ASC LIMIT 1", (now(),)).fetchone()
            if not row:
                return None
            task = dict(row)
...
def execute_one() -> dict[str, Any] | None:
    ...
    if not task:
        return None
    agent = store.one("SELECT * FROM agents WHERE id=? AND enabled=1", (task["agent_id"],)) if task["agent_id"] else None
    if not agent:
    ...
    try:
        ...
        cost = 1
        with store.conn:
            store.conn.execute("UPDATE tasks SET status='done', result_json=?, lease_id=NULL, lease_expires_at=NULL, available_at=NULL, last_error=NULL, updated_at=? WHERE id=? AND lease_id=?", (json.dumps(result), now(), task["id"], task["lease_id"]))
...
app = FastAPI(title="pyPaperclip", version="0.3.1", lifespan=lifespan)
...
def list_agents(company_id: str, limit: int = Query(default=50, ge=1, le=200), offset: int = Query(default=0, ge=0)) -> dict[str, Any]:
    ...
    items = store.many("SELECT * FROM agents WHERE company_id=? ORDER BY created_at LIMIT ? OFFSET ?", (company_id, limit, offset))
    return {"items": items, "limit": limit, "offset": offset, "total": total}

@app.get("/agents/{agent_id}")
def get_agent(agent_id: str) -> dict[str, Any]:
    return require_entity("agents", agent_id, "Agent")
...
def create_task(company_id: str, body: TaskIn) -> dict[str, Any]:
    ...
    item = {"id": new_id("task"), "company_id": company_id, "goal_id": body.goal_id, "agent_id": body.agent_id, "title": body.title, "description": body.description, "priority": body.priority, "idempotency_key": body.idempotency_key, "max_attempts": body.max_attempts, "created_at": now(), "updated_at": now()}
    with store.conn:
        store.conn.execute("INSERT INTO tasks(id,company_id,goal_id,agent_id,title,description,priority,idempotency_key,max_attempts,created_at,updated_at) VALUES(?,?,?,?,?,?,?,?,?,?,?)", tuple(item.values()))
    store.audit(company_id, "task.queued", "task", item["id"], item)
